...
//  rvage
//
//  Created by pkalbers on 10/09/2016.
					  ...
					  const size_t batch_limit = 1000)
{
	...
	const std::string file_pairs = output + ".ccf.txt";
	const std::string file_sites = output + ".cle.txt";
	...
	*/
	...
	const std::string file_pairs_distr = output + ".ccf.distr.txt";
	const std::string file_sites_distr = output + ".cle.distr.txt";
	...
	//std::cout << "  Nucleotide diversity, theta:      " << std::scientific << std::setprecision(4) << param->theta << std::endl;
	...
	try
	{
		...
		while (true)
		{
			...
			for (site = queue.sites.begin(); site != site_end; ++site)
			{
				if ((*site)->done)
				{
					(*site)->print(stream_sites, false, false); // raw
					(*site)->print(stream_sites, false, true);  // adj
					
					if (print_cle_full)
					{
						(*site)->print(stream_sites_distr, true, false); // raw
						(*site)->print(stream_sites_distr, true, true);  // adj
					}
