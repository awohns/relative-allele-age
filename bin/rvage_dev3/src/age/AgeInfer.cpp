...
//  rvage
//
//  Created by Patrick Albers on 07/09/2016.
...
{
	stream << "MarkerID Clock Adjusted";
...
void Site::print(std::ostream & stream, const bool density, const bool print_adj) const
{
	if (!this->done)
	...
	for (int c = 0; c < n_clocks; ++c)
	{
		...
		else
		{
			...
			stream << std::fixed << std::setprecision(8) << out.mean << ' ';
			stream << std::fixed << std::setprecision(8) << out.mode << ' ';
			stream << std::fixed << std::setprecision(8) << out.median << ' ';
